  ...
  return (
    <div className="h-full  flex flex-col">
      <div className=" flex justify-between p-2 items-center text-xl border-b">
        ...
        <span onClick={toggleOpen} className="text-2xl hover:cursor-pointer ">
          X
        </span>
      </div>

      {selectedParent == "" && currentStep == "Select service" && (
          ...
          {choises.map((item, index) => {
            return (
              <motion.div
                key={index}
                onClick={() => {
                  setSelectedParten(item.parent);
                  setParentIndex(item.index);
                }}
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ duration: 0.5, delay: 0.5 }}
                className={`border-2 hover:cursor-pointer hover:border-blue-900 group rounded-lg w-full p-3 flex  justify-between items-center text-xl ${
                  selectedParent == item.parent ? "border-blue-900" : ""
                }`}
              >
                <div className="flex items-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-10 h-10 fill-none stroke-black "
                    viewBox="0 0 384 512"
                    strokeWidth={5}
                  >
          ...
          {choises[parentIndex].children.map((itm, idx) => {
            return (
              <motion.div
                key={idx}
                onClick={() => {
                  setSetlectdChild(itm.name);
                  setChildIndex(idx);
                  itm.hasFixedTime ? "" : setStepNumber(stepNumber + 1);
                }}
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ duration: 0.5, delay: 0.5 }}
                className={`border-2 hover:cursor-pointer hover:border-blue-900 group rounded-lg w-full p-3 flex  justify-between items-center text-xl ${
                  selectedChild == itm.name ? "border-blue-900" : ""
                }`}
              >
                <div className="flex items-center">
                  <svg
      ...
      {selectedParent != "" &&
        selectedChild != "" &&
        choises[parentIndex].children[0].hasFixedTime && (
          <div className="  flex flex-col justify-center items-center gap-5">
            <div className="px-5 pt-5 text-center w-full max-w-sm  mx-auto">
              ...
              <p>
                You need to select service duration , The price of your service
                will depend on duration
              </p>
            </div>
            <div className="flex gap-2">
              {choises[parentIndex].children[childIndex].fixedTimes.map(
                (itm, idx) => {
                  return (
                    <div
                      key={idx}
                      onClick={() => {
                        setSelectedDuration(itm.minutes);
                        setStepNumber(stepNumber + 1);
                      }}
                      className=" hover:border-blue-800 rounded-lg h-28 w-28 flex relative text-xl border-2  hover:cursor-pointer"
                    >
                      <p className="text-blue-900 text-3xl font-semibold text-center  w-fit h-fit m-auto">
                        {itm.price}
                        <p className="text-sm">Minutes</p>
                      </p>
                      <span className="absolute right-1 text-amber-600">
                        ${itm.price}
                      </span>
                    </div>
                  );
    ...
    <div>
      {currentService == "" && (
        <div className="flex flex-col  items-center gap-5  p-5">
          <motion.div
            onClick={() => {
              setCurrentService("General Dentistry");
            }}
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.5 }}
          ...
          <motion.div
            onClick={() => {
              setCurrentService("Psychologie servie");
            }}
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.5 }}
          ...
          <motion.div
            onClick={() => {
              setCurrentService("Massage and recovery");
            }}
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.5 }}
...
const Denstist = () => {
  ...
  return (
    <div className="flex flex-col  items-center gap-5  p-5">
      ...
      >
        <div className="flex items-center gap-3">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512 512"
            className="w-6 h-6 fill-none stroke-black "
            strokeWidth={2}
          >
      ...
      <motion.div
        onClick={() => {
          setDesiredAction("Invisilign Braces");
        }}
        initial={{ y: 20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.5, delay: 0.5 }}
      ...
      <motion.div
        onClick={() => {
          setDesiredAction("Teeth Whitening");
        }}
        initial={{ y: 20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.5, delay: 0.5 }}
